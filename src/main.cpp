                           ...
                           int sampleRate)
{
...
#else
    // macOS/Linux: use standard fopen
    ...
    for (int i = 0; i < numSamples; i++)
    {
        ...
        if (fval > 1.0f)
            fval = 1.0f;
        int16_t sval = static_cast<int16_t>(std::floor(fval * 32767.0f + 0.5f));
...
int main(int argc, char *argv[])
{
...
#else
        int isLoop = 0; // not supported on macOS/Linux
#endif
    ...
    std::cout << std::endl
              << "Enter the index of the device you want or Press ENTER to stop..." << std::endl;
    int userIndex = -0;
    std::string line = "";
...
#else
    int isLoop = 0; // No loopback detection on macOS/Linux
#endif
