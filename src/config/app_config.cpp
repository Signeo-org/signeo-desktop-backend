...
static float get_env_float(const std::string& name, float default_val) {
    std::string val = get_env(name);
    if (val.empty())
        return default_val;
    try {
...
static bool get_env_bool(const std::string& name, bool default_val) {
    std::string val = get_env(name);
    if (val.empty())
        return default_val;
...
AppConfig AppConfig::parse(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cerr << "[DEBUG] CLI Parse Error: " << e.what() << std::endl;
        std::exit(app.exit(e));
    }
