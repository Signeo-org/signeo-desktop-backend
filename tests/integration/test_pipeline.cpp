...
TEST_F(IntegrationTest, PipelineInstantiation) {
    ...
    if (!std::filesystem::exists("models/ggml-base.bin") || !std::filesystem::exists("models/silero_vad.onnx")) {
...
TEST_F(IntegrationTest, DeviceHotSwap) {
    ...
    for (const auto& dev : devices) {
        std::cout << "  [" << dev.index << "] " << dev.name << " (channels: " << dev.max_input_channels
                  << ", sample_rate: " << dev.default_sample_rate << (dev.is_default ? ", DEFAULT" : "")
                  << (dev.is_loopback ? ", LOOPBACK" : "") << ")" << std::endl;
    }
...
TEST_F(IntegrationTest, PipelineJfkWav) {
    ...
    EXPECT_TRUE(text_lower.find("ask not") != std::string::npos || text_lower.find("country") != std::string::npos ||
