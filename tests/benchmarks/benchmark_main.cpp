...
private:
    ...
    void print_results(int iterations, double total_ms, double mean_us, 
                       double stdev_us, double p50, double p95, double p99) {
        // Record to XML
        BenchmarkRecord rec;
        rec.name = name_;
...
public:
    static void run_contention_test() {
        ...
        // Test 1: Mutex contention
        {
            ...
            std::cout << "  Mutex Contention (" << NUM_THREADS << " threads, " 
        ...
        // Test 2: Atomic contention
        {
            ...
            std::cout << "  Atomic Contention (" << NUM_THREADS << " threads, " 
                      << OPERATIONS_PER_THREAD << " ops each):\n";
        ...
        // Test 3: ThreadSafeQueue throughput
        {
            ...
            std::cout << "  ThreadSafeQueue (" << NUM_THREADS << " producers, 1 consumer, " 
                      << OPERATIONS_PER_THREAD << " items each):\n";
            std::cout << "    Total time:       " << duration_ms << " ms\n";
...
void benchmark_throughput() {
    ...
    std::cout << "  Speed Factor:      " << std::setprecision(2) << (simulated_audio_duration_s / (elapsed_ms / 1000.0)) << "x realtime\n";
...
int main(int argc, char** argv) {
    ...
    std::vector<WerTestCase> wer_tests = {
        {"Perfect Match", 
         ...
         "the quick brown fox jumps over the lazy dog"},
        {"Minor Errors (typical STT)", 
         "the quick brown fox jumps over the lazy dog",
         "the quick brown fox jumps over lazy dog"},
        {"Moderate Errors", 
         "hello world how are you doing today",
         "hello word how are you do today"},
        {"Significant Errors", 
         "speech recognition is very accurate now",
         "speech wreck ignition is vary accurate"},
        {"Homophones", 
         "I would like to eat there today",
         "I wood like too eat their today"}
    };
    
    for (const auto& test : wer_tests) {
        auto result = utils::WerCalculator::calculate(test.reference, test.hypothesis);
        std::cout << "  " << test.name << ":\n";
        std::cout << "    WER: " << std::fixed << std::setprecision(1) 
                  << result.wer_percentage() << "%";
        std::cout << " (S:" << result.substitutions 
                  << " D:" << result.deletions 
                  << " I:" << result.insertions << ")\n";
    ...
    // Memory delta
    std::cout << "\n[MEMORY] Delta\n";
    std::cout << "  Working Set Change: " << (static_cast<long long>(mem_end.working_set_kb) - static_cast<long long>(mem_start.working_set_kb)) << " KB\n";
